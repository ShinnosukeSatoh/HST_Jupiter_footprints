...
# equivalent values previously used. These are given in the arrays below
        ...
        if cr != 2.5:
            ...
            newfactors = {'F115LP': f1, 'F125LP': f2, 'CLEAR': cl, 'F25SRF2': sr}\
                         [self.alm.filter]
...
# x and y are logical coordinates based on the full 1440 x 720 array (0.25 deg resolution)
        if self.alm.hemisph == 'north':
            if self.ny == 180:
                ...
                print('Unrecognised cylindrical projection array shape:', self.nx, self.ny)
                sys.exit()
...
# Planetary radius set to 1 R_p. Oblateness was taken care of in reduction
        self.globe=ccrs.Globe(semimajor_axis=1, semiminor_axis=1, ellipse=None)
...
# Output projection
        ...
        if hem == 'north':
            if reflon is None: reflon = 180
            outproj = ccrs.NorthPolarStereo(central_longitude=reflon, globe=self.globe)
        else:
            if reflon is None: reflon = 0
            outproj = ccrs.SouthPolarStereo(central_longitude=reflon + 180, globe=self.globe)
...
# Show the image and plot gridlines if required
        ...
        self.tvim = ax.imshow(dimage, origin='lower', extent=out_extent, norm=self.normfunc, cmap=cmap) 
...
# Load and plot the satellite contours
        ...
        if len(satovals) > 0:
            ...
            if 'io' in satovals:
                ax.plot(satoval.iowlon, satoval.iolat, 'w', transform=self.geodetic, lw=0.4)                
            if 'eu' in satovals:
                ax.plot(satoval.euwlon, satoval.eulat, 'w', transform=self.geodetic, lw=0.4)   
            if 'ga' in satovals:               
                ax.plot(satoval.gawlon, satoval.galat, 'w', transform=self.geodetic, lw=0.4)
...
# Show the image and plot gridlines if required
        self.normfunc = {'log': LogNorm(vmin=vmin, vmax=vmax),
                    'lin': Normalize(vmin=vmin, vmax=vmax)}[norm]
        self.tvim = ax.imshow(dimage, origin='lower', extent=out_extent, norm=self.normfunc, cmap=cmap, transform=self.cylproj)
        ax.set_aspect('equal', adjustable='box')
        if grid is True:
            gl = ax.gridlines(xlocs=np.arange(-180, 180, grid_spacing[0]), 
                              ylocs=np.arange(-90, 90, grid_spacing[1]), 
