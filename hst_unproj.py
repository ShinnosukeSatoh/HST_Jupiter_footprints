...
###############################################################################
    ...
    def tvPolar(self, axis=None, vmin=None, vmax=None, yclip=88, yclipval=None,
        ...
        if hem == 'north':
            if reflon is None:
                reflon = 180
            outproj = ccrs.Orthographic(
                central_longitude=reflon, central_latitude=0,
                globe=self.globe)
        else:
            if reflon is None:
                reflon = 0
        ...
        out_extent = np.array([-0.999, 0.999, -0.999, 0.999])
        if hem == 'north':
            out_extent = np.array([-0.65, 0.65, 0.55, 0.999])
        else:
            out_extent = np.array([-0.65, 0.65, -0.999, -0.55])

        # Perform the image transformation
        dimage, dum = warp_array(dimage, outproj, source_proj=self.cylproj,
                                 target_res=(1000, 1000),
                                 source_extent=self.cylproj_extent,
